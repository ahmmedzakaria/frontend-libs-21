...
/**
 * `DynamicDropdownMode: 'api-simple'` implementation — one debounced fetch per
 * query, always page 0, no scroll pagination. One of the three concrete
 * dropdown components `DynamicDropdownComponent` delegates to based on its
 ...
 * binding, since `DynamicDropdownComponent` is the sole form-integration point
 * for all three modes.
 */
@Component({
...
export class ApiSimpleDropdownComponent<T = string> {
    ...
    private fetchPage(query: string): Observable<DynamicDropdownPage<T>> {
